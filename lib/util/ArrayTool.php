...
class ArrayTool
{
    public static function findAllCodesByCode($code)
    {
        $codes = explode('.', $code);
        $allCodes = array();
        $codeString = '';
    ...
    public static function groupParts(array $array, $column, $partKeys)
    {
        $grouped = array();

        foreach ($array as $item) {

            $newKey = empty($item[$column])?'':$item[$column];
            $keys = array_keys($item);
            foreach ($keys as $key) {
                if (!in_array($key, $partKeys)) {
                    unset($item[$key]);
                }
            }
    ...
    public static function groupConcat(array $array, $key, $index, $column)
    {
        $grouped = array();

        foreach ($array as $item) {
            if (empty($grouped[$item[$key]][$item[$index]])) {
                $grouped[$item[$key]][$item[$index]] = array();
            }

            $grouped[$item[$key]][$item[$index]][] = $item[$column];
        }
    ...
    public static function columnKeyVal(array $array, $val_colcumn, $key_column = null)
    {
        $returnArr = array();
        
        foreach ($array as $key => $item) {
            
            if( ($key_column && !isset($item[$key_column]))
                              || !isset($item[$val_colcumn]) ){
                continue;
            }

            $returnArr[$key_column?$item[$key_column]:$key] = $item[$val_colcumn];
        }

        return $returnArr;
    }
    
    public static function explodeVal(array $array, $delimiter=',')
    {
        
        foreach ($array as $key => $value) {
            $array[$key] = explode($delimiter, trim($value,$delimiter));
        }
